...
namespace StateMachineBuddy
{
	...
	public class StateMachine : IStateMachine<int>
	{
		...
		public void SetMessageName(int message, string messageName)
		{
			...
			MessageNames[adjustedMessage] = messageName;
		}
		...
		public string GetMessageName(int message)
		{
			...
			return MessageNames[adjustedMessage];
		}
		...
		private void ReadNames(List<string> listStateNames, List<string> listMessageNames)
		{
			...
			if (numNewStates > 0)
			{
				...
				for (var i = 0; i < listStateNames.Count; i++)
				{
					...
					if (-1 == stateIndex)
					{
						StateNames[nextBlankState] = listStateNames[i];
						nextBlankState++;
					}
				}
			}
			...
			if (numNewMessages > 0)
			{
				...
				for (var i = 0; i < listMessageNames.Count; i++)
				{
					...
					if (-1 == messageIndex)
					{
						MessageNames[nextBlankMessage] = listMessageNames[i];
						nextBlankMessage++;
					}
				}
			}
